## extra_mangle_props.js -- Do a bunch more ugly JS compression.
...
def compress_common_names(data):
    ...
    new_toks = [{x: ("_"*10+x)[-8]+x[-2]+x[3] for x in x.split() if len(x) >= 4} for x in open("rebind_props").readlines()]
    ...
    for line in open("rebind_props").readlines()[-2:]:
        for x in line.split():
            skip_replaces.add("."+x)
...
# 3251
# 3284 (stars are now one cube)
# 3417 (no more still objects or merge sprites)
# 3682 (no more subdivisioning)
# 3804 (simplify shaders)
# 3925 (more simplify shaders)
# 3939 (shaders again)
# 4435 (HUD -> webgl logic)
# 5120 (remove filtering)
# 5225 (better objects)
# 4696 (making things pretty)
# 4968 (remove lathe)
# 5081 (remove make output from faces)
# 4865 (better fighting)
# 4750 (more shader compression)
# 4790 (single particle system)
# 3441 (many more game objects)
# 3756 (better object compression)
# 4046 (shader compression again)
# 4501 (camera compression, variables)
# 4566 (more particle system)
# 3590 (better enemy interaction)
# 3148 (improved loading screen)
# 2704 (particles stick to objects)
# 2813 (condense lazer code)
# 2936 (make camera a function)
# 3382 (better camera, shader)
# 3516 (cut down shaders, graphics)
# 3582 (more minor tweaking)
# 3108 (sounds)
# 3295 (object loading, lazer movement)
# 3134 (more efficient particles)
# 2677 (protect the star)
# 245 (ready)
# 186 (aimbot)


def compress():
    files = ["jsfxr.js", "audio.js", "utils.js", "objects.js", "graphics.js", "game.js", "webgl.js", "dat.js"]
    ...
    """
    ### ATTEMPT 2
    src="gl.FLOAT, gl.TEXTURE0, gl.RGBA32F, gl.RG32F, gl.TEXTURE_WRAP_T, gl.TEXTURE_WRAP_S, gl.TEXTURE20, gl.FRONT, gl.DYNAMIC_DRAW, gl.DEPTH_ATTACHMENT, gl.COLOR_ATTACHMENT0".split(", ")
    ...
    open("build/comp3.js","w").write(data.replace("zzHACK","").replace("}^", "} "))
    ...
    open('build/index.html',"w").write(open("src/webglx.html").read()[:-58]+"<script>"+open("build/comp3.js").read().replace("QWQWQ","").replace("zzHACK","")+"</script>")
    ...
    used = int(os.popen("advzip -i 100 -4 -a /tmp/foo.zip build/index.html && wc /tmp/foo.zip").read().split()[-2])
